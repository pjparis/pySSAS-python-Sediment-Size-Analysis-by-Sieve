...
class SedSAS(object):
	...
	def ComputeLogGraphicStats(self,s,Q, prt2stdout=False ):
		'''computes "graphic" mean, median, standard deviation, skewness, and kurtosis 
		   ...
		   Input args:
			   ...
			   prt2stdout  True, False (default) Option to print results to std out
			      call to internal method PrintGraphicStats()
		...
		if( prt2stdout==True):
			self.PrintGraphicStats(s, (MN,MD,SD,SK,K), 'phi' )
			
		return( (MN,MD,SD,SK,K) )



	def ComputeGeomGraphicStats(self,s,Q, prt2stdout=False ):
		...
		MD=np.log(2**Q[4] )					 # for the graphic median:
	...
	def FindSampleModes(self, s, prt2stdout=False):
		'''locates any modal weight percentage values in current sample. 

		   Input args: 
			   s = sample identifier as a Python string (EX. 'S1)
			   prt2stdout True/False(default) gives user the option to print results to
		...
		for i in range(len(self.df[s+'wp'])):
			...
			# for others...
				...
				#if the user elects to print the modes (in addition to returning them in
				# the dictionary) to std out:
		if(prt2stdout==True):
			print('Mode(s) for:', s)
			print('Phi', ' Weight %')
			print('-------------------')
	...
	def PrintGraphicStats(self, s, gStats, units='phi' ):  
		'''prints the graphic mean, median, standard deviation, skewness, and kurtosis 
			...
			Input args:
				...
				units - phi (default), mm. The particle size units. When function called
				        from ComputeLogGraphicStats units are phi, when from ComputeGeom-
				        GraphicStats the units are mm.

			Returns: none

			 Note: 1.) this is an internal function, optionally called by ComputeLogGraphicStats
			       or ComputeGeomGraphicStats to print particle size statistics to std
			       out
			       
			       2.) the gStats tuple is generated by method self.ComputeGraphicStats()
		'''
		print('-'*80)
		...
		if np.isnan(gStats[0]):
			...
			print('Est. Graphic Mean', round(gStats[0],2), units )

		if np.isnan(gStats[1]):
			print('Est. Graphic Median:','Out of Range*')
		else:
			print('Est. Graphic Median', round(gStats[1],2), units )

		if np.isnan(gStats[2]):
			print('Est. Graphic Standard Deviation:','Out of Range*')
		else:
			print('Est. Graphic Standard Deviation', round(gStats[2],2), units )
